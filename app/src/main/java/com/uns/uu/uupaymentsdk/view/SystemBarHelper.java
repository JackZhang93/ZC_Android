...
//    private static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? 0.15f : 0.15f;
     ...
     */
    public static void tintStatusBar(Activity activity, @ColorInt int statusBarColor, boolean visible) {
        tintStatusBar(activity, statusBarColor, DEFAULT_ALPHA, visible);
    }
     ...
     */
    public static void tintStatusBar(Activity activity, @ColorInt int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha, boolean visible) {
        tintStatusBar(activity.getWindow(), statusBarColor, alpha, visible);
    }
     ...
     * @param statusBarColor 状态栏颜色
     */
    public static void tintStatusBar(Window window, @ColorInt int statusBarColor, boolean visible) {
        tintStatusBar(window, statusBarColor, DEFAULT_ALPHA, visible);
    }
...
//        setStatusBar(decorView, statusBarColor, true, true);
     ...
     * android 6.0设置字体颜色
     *
     * @param dark true状态栏字体颜色设置为黑色，false字体颜色显示默认颜色
     */
    ...
    public static void setStatusBarDarkModeForM(Window window, boolean dark) {
        ...
        window.getDecorView().setSystemUiVisibility(dark ? systemUiVisibility : 0);
    }
...
//        setStatusBar(container, statusBarColor, visible, false);
     ...
     * 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的
     */
    public static void setCancelPadding(Context context, View view) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            lp.height = 0;//增高
     ...
     *                        沉浸式状态栏：colorResId设置为0，stateBisible设置为false
     * @param dark            设置状态栏字体颜色是否变黑
     */
    public static void setStateBarColor(Activity context, int colorResId, boolean stateBarVisible, boolean dark) {
        int color = 0;
        if (colorResId != 0) {
            color = ContextCompat.getColor(context, colorResId);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            SystemBarHelper.setStatusBarDarkModeForM(context.getWindow(), dark);
            SystemBarHelper.tintStatusBar(context, color, 0, stateBarVisible);
        } else if (SystemBarHelper.isFlyme4Later()) {
            SystemBarHelper.setStatusBarDarkModeForFlyme4(context.getWindow(), dark);
            SystemBarHelper.tintStatusBar(context, color, 0, stateBarVisible);
        } else if (SystemBarHelper.isMIUI6Later()) {
            SystemBarHelper.setStatusBarDarkModeForMIUI6(context.getWindow(), dark);
            SystemBarHelper.tintStatusBar(context, color, 0, stateBarVisible);
        } else {
            SystemBarHelper.tintStatusBar(context, color, stateBarVisible);
        }
    }
}